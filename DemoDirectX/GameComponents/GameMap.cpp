...
#pragma region -ENEMY-
	for (size_t i = 0; i < GetMap()->GetNumTileLayers(); i++)
	{
		...
		//xac dinh layer Enemy bi an di de tu do tao ra cac vien gach trong game, nhung vien gach khong phai la 1 physic static nos co the bi pha huy duoc

		if (layer->GetName() == "enemy")
		{
				...
				for (size_t m = 0; m < layer->GetHeight(); m++)
				{
					for (size_t n = 0; n < layer->GetWidth(); n++)
					{
						if (layer->GetTileTilesetIndex(n, m) != -1)
						{
							...
							enemy->Tag = Entity::EntityTypes::Enemy;
							mListEnemies.push_back(enemy);


							if (enemy)
								mQuadTree->insertEntity(enemy);
						}
					}
				}
			}
...
#pragma region DRAW BRICK
    ...
    for (size_t i = 0; i < mListBricks.size(); i++)
    {
		if (mListBricks[i]->isDestroy == false)
...
#pragma region DRAW ENEMY
	...
	for (size_t i = 0; i < mListEnemies.size(); i++)
	{
		if (mListEnemies[i]->isDestroy == false)
			mListEnemies[i]->Draw(trans);
	}
...
std::vector<Enemy*> GameMap::GetListEnemy()
{
	return mListEnemies;
}
